...
#ifndef P
#define P 12          // FMM expansion order (<= 12: BIN / CKJ are 12 x 12)
#endif
#ifndef CDF
    ...
    for (int l = 1; l < nlev; ++l) {
        ...
        for (int b = tid; b < nbx; b += nth) {
            ...
            for (int ch = c1; ch <= c2; ++ch) {
                ...
                for (int k = 0; k < P; ++k)
                    #pragma unroll
                    for (int j = 0; j <= k; ++j) fma2(Mp[k], CKJ[k * 12 + j] * bp[k - j], A[j]);
            }
            #pragma unroll
        ...
        for (int b = tid; b < nbx; b += nth) {
            ...
            for (int cand = max(c0, 0); cand <= min(c1, nbx - 1); ++cand) {
                ...
                for (int j = 0; j < P; ++j) {
                    ...
                    #pragma unroll
                    for (int k = 0; k < P; ++k) fma2(s2, BIN[j * 12 + k], A[k]);
                    fma2(La[j], bj, s2);
                    bj *= bb;
                }
            }
    ...
    for (int l = nlev - 2; l >= 0; --l) {
        ...
        for (int b = tid; b < nbx; b += nth) {
            ...
            for (int i = 0; i < P; ++i) {
                ...
                #pragma unroll
                for (int j = i; j < P; ++j) fma2(acc, CKJ[j * 12 + i] * bp[j - i], Lp[j]);
